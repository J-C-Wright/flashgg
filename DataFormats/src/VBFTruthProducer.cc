...
VBFTagTruth VBFTruthProducer::produce( unsigned int diPhotonIndex ) {
    ...
    edm::Ptr<flashgg::DiPhotonCandidate> diPhoton = diPhotonCollection_->ptrAt(diPhotonIndex);
    truthObject.setDiPhoton(diPhoton);

//GenPartcle level
    //Find the partons
    std::vector<edm::Ptr<reco::GenParticle>> partons;
    for (unsigned int genLoop(0);genLoop < genParticles_->size();genLoop++) {
        edm::Ptr<reco::GenParticle> gp = genParticles_->ptrAt(genLoop);
...
//GenJet Level
    ...
    for( unsigned int jetLoop( 0 ); jetLoop < genJets_->size(); jetLoop++ ) {
        edm::Ptr<reco::GenJet> gj = genJets_->ptrAt( jetLoop );
...
//FLASHgg Jet Level
    //Pt-ordered fggJets
    Handle<View<flashgg::Jet>> fggJets = jetCollections_[diPhoton->jetCollectionIndex()]; 
...
//Closest matches to FLASHgg jets and the Diphoton
    ...
    if (ptOrderedFggJets.size() > 0) {
        float dr(999.0);
        unsigned gpIndex(0);
        for (unsigned partLoop(0);partLoop<genParticles_->size();partLoop++) {
            edm::Ptr<reco::GenParticle> particle = genParticles_->ptrAt(partLoop);
        ...
        truthObject.setClosestParticleToLeadingJet(genParticles_->ptrAt(gpIndex));
    } 
        //Sublead
    if (ptOrderedFggJets.size() > 1) {
        ...
        truthObject.setClosestParticleToSubLeadingJet(genParticles_->ptrAt(gpIndex));
    } 
        //Subsublead
    if (ptOrderedFggJets.size() > 2) {
        ...
        truthObject.setClosestParticleToSubSubLeadingJet(genParticles_->ptrAt(gpIndex));
    } 
    ...
    if (ptOrderedFggJets.size() > 0) {
        float dr(999.0);
        unsigned gjIndex(0);
        for (unsigned jetLoop(0);jetLoop<ptOrderedFggJets.size();jetLoop++) {
    ...
    truthObject.setClosestParticleToLeadingPhoton(genParticles_->ptrAt(gpIndex1));
    truthObject.setClosestParticleToSubLeadingPhoton(genParticles_->ptrAt(gpIndex2));
