...
def get_options():
    ...
    parser.add_option('--signalRegion',
                      ...
                      between 115 and 135
                      ''')
    parser.add_option('--funcName',
                      dest='functionName',
                      default='',
                      help='''
                      Specify the name of the function for the 
                      naming of tables and directories.
                      ''')
    parser.add_option('--function',
                      dest='function_expression',
                      default='',
                      help='''
                      Specify the function to be used with (x,y,z)
                      for the (up to 3) variables.
                      ''')
...
if __name__ == '__main__':
    ...
    funcName = opt.functionName


    outpath = '/afs/cern.ch/user/j/jwright/www/vbf_sep_'+funcName+'_'+cut_name+'_'+varsInfoFile.split('.')[0]+'/'
    if not os.path.exists(outpath):
        os.makedirs(outpath)
    copyfile('/afs/cern.ch/user/j/jwright/www/index.php',outpath+'index.php')

    tableName = 'SepPower_'+funcName+'_'+cut_name+'_'+varsInfoFile.split('.')[0]
    ...
    for i in range(len(histos_info)):
        for j in range(len(histos_info)):
            ...
            variables.append(opString % tuple([var1_info[4],var1_info[4],var2_info[4]]))

    for bkg_trees,bkg_name in zip(bkg_channel_trees,bkg_names):

        print "\n\nProcessing vbf vs ",bkg_name
        ...
        for i in range(len(histos_info)):
            for j in range(len(histos_info)):
                ...
                label_newVar = opString % tuple([var1_info[4],var1_info[4],var2_info[4]])
...
#    highlights = [row[5] for row in histos_info]
    ...
    highlights = ['False']*len(variables)
    print all_sort
