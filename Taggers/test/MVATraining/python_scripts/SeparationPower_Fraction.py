...
if __name__ == '__main__':
    ...
    outpath = '/afs/cern.ch/user/j/jwright/www/vbf_sep_Fraction_'+cut_name+'_'+varsInfoFile.split('.')[0]+'/'
    if not os.path.exists(outpath):
        os.makedirs(outpath)
    copyfile('/afs/cern.ch/user/j/jwright/www/index.php',outpath+'index.php')

    tableName = 'SepPower_Fraction_'+cut_name+'_'+varsInfoFile.split('.')[0]
    ...
    for i in range(len(histos_info)):
        for j in range(len(histos_info)):
            ...
            variable = '\\sqrt{{%s}^{2} + {%s}^{2}}' % (var1_info[-1],var2_info[-1])
    ...
    for bkg_trees,bkg_name in zip(bkg_channel_trees,bkg_names):
        ...
        for i in range(len(histos_info)):
            for j in range(len(histos_info)):
                ...
                if i >= j: continue
                newVar = opString % (var1_info[0],var1_info[0],var2_info[0])
                ...
                signal_histo,bkg_histo = vs.getCombinationVarSBHistoPair(signal_trees=vbf_trees,bkg_trees=bkg_trees,
                                                                         ...
                                                                         opString=opString,cut=cut,whiten=True)
                ...
                label_newVar = '\\sqrt{{%s}^{2} + {%s}^{2}}' % (var1_info[-1],var2_info[-1])
